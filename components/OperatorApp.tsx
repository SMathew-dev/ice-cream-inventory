...
import { FreezerMapPanel } from './FreezerMapPanel';
import { HardCountCard } from './HardCountCard';
import { LabReleasePlacement } from './LabReleasePlacement';
import { OrderPickupQueue } from './OrderPickupQueue';
import { ProductionPanel } from './ProductionPanel';
import { useFreezerPlacements } from '../lib/hooks/useFreezerPlacements';
import { useFreezerSlots } from '../lib/hooks/useFreezerSlots';
import { useInventoryActions } from '../lib/hooks/useInventoryActions';
import { useInventoryData } from '../lib/hooks/useInventoryData';
import { useOperatorQueues } from '../lib/hooks/useOperatorQueues';
import { getSupabaseBrowserClient } from '../lib/db/supabase';

type Screen='dashboard'|'orders'|'production'|'dairy'|'count'|'storefront'|'freezer';

export function OperatorApp(){
  const {inventory,mode,loading,error,refresh}=useInventoryData();
  const queues=useOperatorQueues();
  const slots=useFreezerSlots();
  const freezer=useFreezerPlacements();
  const [screen,setScreen]=useState<Screen>('dashboard');

  const refreshAll=useCallback(async()=>{
    await Promise.all([refresh(),queues.refresh(),slots.refresh(),freezer.refresh()]);
  },[refresh,queues.refresh,slots.refresh,freezer.refresh]);
  const actions=useInventoryActions(refreshAll);
  ...
  const title={dashboard:'Inventory Control',orders:'Customer Orders',production:'Production & Lab',dairy:'Dairy Bar',count:'Hard Count / Fix Number',storefront:'Storefront Inventory',freezer:'Freezer Map'}[screen];

  return <main className="shell">
    <aside className="sidebar">
      <div className="brand">ICE CREAM<br/><span>INVENTORY</span></div>
      <nav>
        {([['dashboard','Dashboard'],['orders','Customer Orders'],['production','Production / Lab'],['dairy','Dairy Bar'],['freezer','Freezer Map'],['count','Hard Count'],['storefront','Storefront']] as const).map(([key,label])=><button key={key} className={screen===key?'active':''} onClick={()=>setScreen(key)}>{label}</button>)}
      </nav>
      <div className="sidebar-note">Product identity:<strong> Julian · flavor · package</strong><button onClick={()=>void getSupabaseBrowserClient().auth.signOut()}>Sign out</button></div>
    </aside>
    ...
    <section className="content">
      ...
      {(error||queues.error||slots.error||freezer.error)&&<div className="notice">Database connection problem: {error||queues.error||slots.error||freezer.error}</div>}
      ...
      {screen==='dashboard'&&<>
        ...
        <section className="card inventory-card"><div className="card-head"><div><p className="eyebrow">LIVE INVENTORY</p><h2>Saleable stock</h2></div><button onClick={()=>void refreshAll()}>Refresh</button></div><InventoryTable inventory={inventory}/></section>
      ...
      {screen==='orders'&&<div className="grid2"><CustomerOrderPanel inventory={inventory} onChanged={refreshAll}/><OrderPickupQueue orders={queues.openOrders} onComplete={actions.completeOrder} onCancel={actions.cancelOrder}/></div>}
      {screen==='production'&&<div className="grid2"><ProductionPanel inventory={inventory} onChanged={refreshAll}/><LabReleasePlacement lots={queues.pendingLots} slots={slots.slots} onPass={actions.releaseAndPlaceLot} onFail={actions.failLabLot}/></div>}
      {screen==='dairy'&&<DairyBarCart inventory={inventory} onSubmit={actions.withdrawDairyBar}/>} 
      {screen==='freezer'&&<FreezerMapPanel placements={freezer.placements}/>} 
