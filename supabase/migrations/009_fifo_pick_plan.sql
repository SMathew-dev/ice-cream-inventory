...
returns table(placement_id uuid,production_run_id uuid,julian integer,quantity_to_pull integer,wall text,shelf text,slot_position integer,position_label text)
...
begin
  ...
  for r in
    select fp.id placement_id,fp.production_run_id,pr.julian_day::int julian,fp.quantity,fs.wall,fs.shelf,fs.position as slot_position,fs.position_label
    from freezer_placements fp join freezer_slots fs on fs.id=fp.slot_id join production_runs pr on pr.id=fp.production_run_id
    where fp.product_id=p_product_id and fp.quantity>0 and fs.freezer='-20°F' and pr.status='PASSED'
  ...
  loop
    ...
    wall:=r.wall; shelf:=r.shelf; slot_position:=r.slot_position; position_label:=r.position_label;
