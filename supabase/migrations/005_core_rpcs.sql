...
declare v_product uuid; v_run uuid := gen_random_uuid(); v_good integer;
begin
...
begin
  ...
  if r.good_quantity>0 then
    insert into inventory_movements(product_id,movement_type,quantity,reference_type,reference_id,reason,idempotency_key)
    values(r.product_id,case when p_result='PASS' then 'LAB_RELEASED'::movement_type else 'LAB_FAILED_DISPOSAL'::movement_type end,r.good_quantity,'PRODUCTION_RUN',r.id,case when p_result='PASS' then 'Lab pass to -20°F saleable' else 'Lab failed; disposed' end,'release:'||r.id::text)
...
begin
  ...
  if coalesce(jsonb_array_length(p_items),0)=0 then raise exception 'Order requires at least one item'; end if;
...
returns void language plpgsql as $$
declare v_current integer; v_delta integer; v_adjustment uuid:=gen_random_uuid();
begin
  ...
  insert into inventory_movements(product_id,movement_type,quantity,reason,reference_type,reference_id,idempotency_key)
  values(p_product_id,case when v_delta>0 then 'ADJUSTMENT_IN'::movement_type else 'ADJUSTMENT_OUT'::movement_type end,abs(v_delta),coalesce(nullif(trim(p_reason),''),'Count correction'),'HARD_COUNT',v_adjustment,'count:'||v_adjustment::text);
end; $$;
